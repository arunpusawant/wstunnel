...
module.exports = (wst_server = class wst_server {
  ...
  constructor(dstHost, dstPort, webapp) {
    ...
    this.wsServer = new WebSocketServer({
      ...
      fragmentationThreshold:100000000,
        ...
        return this.authenticate(httpRequest, (rejectReason, target, monitor) => {
          ...
          var tcpConn = net.connect({host, port, allowHalfOpen: false}, () => {
            ...
            try {
              ...
              log(`Client ${ip} established ${request instanceof HttpTunnelReq ? 'http' : 'ws'} tunnel to ${host}:${port}`);
            } catch (e) {
          ...
          tcpConn.on("error", err => request.reject(500, JSON.stringify(`Tunnel connect error to ${host}:${port}: ` + err)));
        });
      };
